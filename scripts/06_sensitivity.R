...
sbp1 <- import("results/res_fastlm.24SBP_multi_model.tsv")
dbp1 <- import("results/res_fastlm.24DBP_multi_model.tsv")
sysSD1 <- import("results/res_fastlm.sysSD_multi_model.tsv")
diaSD1<- import("results/res_fastlm.diaSD_multi_model.tsv")
...
mgs <- data[, grep("HG3A.",names(data),value=T)]
cov <- data[, c("age","gender", "q005a","siteid_plate", "cur_smoke","Fibrer","Energi_kcal","diab_treat","HC_treat","sodium_kawa")]
# antibiotics
yi="sbp_m_all"
...
res$id=substring(res$var.x,nchar(res$var.x)-8,nchar(res$var.x))
