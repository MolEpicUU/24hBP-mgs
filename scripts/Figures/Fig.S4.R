...
data$mgs=gsub("Dysosmobacter sp001916835 hMGS.00195\nPrevalence = 84.7%","*Dysosmobacter* sp001916835 hMGS.00195\nPrevalence = 84.7%",data$mgs)
data$mgs=gsub("Streptococcus sp001556435 hMGS.01022\nPrevalence = 72.2%","*Streptococcus* sp001556435 hMGS.01022\nPrevalence = 72.2%",data$mgs)
data$mgs=gsub("Intestinimonas massiliensis hMGS.01559\nPrevalence = 74.7%","*Intestinimonas massiliensis* hMGS.01559\nPrevalence = 74.7%",data$mgs)

data$mgs <- gsub("\n", "<br>", data$mgs, fixed = TRUE)


p1=ggplot(data, aes(x = type, y = diff, color = p)) + geom_hline(yintercept = 0, size = 0.2) + geom_point(size = 2) + facet_wrap_paginate(~ trait+mgs, nrow=3,ncol = 3, scales = "free",drop=T) + lims(y = c(-0.6, 0.6)) + theme_classic() + theme(line = element_line(size = 0.2), text = element_text(size = 9), strip.clip = "off",strip.placement="outside",strip.background = element_blank(),  strip.text = ggtext::element_markdown(), panel.spacing.x = unit(2, "lines"), panel.spacing.y = unit(4, "lines"),legend.text = element_markdown()) + scale_color_manual(values = safe_colorblind_palette[c(8, 10)]) + labs(x = NULL, y = "(sensitivity analysis estimate - model 1 estimate) / model 1 estimate", color = NULL)

pages=n_pages(p1)

pdf("results/figures/Fig.S4.pdf", width = 8.3, height = 11.7)
for(i in 1:pages){
print(i)
