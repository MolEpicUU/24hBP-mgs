...
ds=import("raw/dsMGS_relative_abundance_4838_upp_4980_malmo.tsv")
...
############################################
########change the name of taxonomy#########
############################################
#deal with the taxonomy data (update the taxonomy data here)
colnames(tax.info)[1] <- "mgs"
colnames(tax.info)[1]
tax.info=tax.info[,c(1,2)]
tax.info$maintax_mgs=paste(tax.info$MainTax, tax.info$mgs, sep="____")
tax.info$maintax_mgs=sub(" ", "_", tax.info$maintax_mgs)
tax.info$maintax_mgs=sub("-", "_", tax.info$maintax_mgs)
tax.info$maintax_mgs=sub(" ", "_", tax.info$maintax_mgs)
tax.info$maintax_mgs=sub("/", ".", tax.info$maintax_mgs)
...
ds$shannon=diversity(ds[,3:ncol(ds)],index="shannon")
dades=merge(dades,ds[,c("id","shannon")],by="id",all.x=T)
...
##################################################################
#MGS filter <30 non-zero values (microbiome)
#select only mgs columns
dades.mgs=dades.abpm[,grep("___",names(dades.abpm),value=T)]
#apply the filter
mgs.filter=apply(dades.mgs, 2, function(x) table(x!=0)["TRUE"]<30)
#show how mgs will be removed (will be removed the true mgs)
#table(mgs.filter)
#select the mgs that will be removed
mgs.filter=names(mgs.filter[mgs.filter%in%"TRUE"])
#QC checks
#remove the selected mgs from dades.mgs
dades.mgs=dades.mgs[,!names(dades.mgs)%in%mgs.filter]
#check that any mgs will sum 0
#table(colSums(dades.mgs)==0)
#check that remaining mgs will have >30 non zero values
#apply the filter
mgs.filter2=apply(dades.mgs, 2, function(x) table(x!=0)["TRUE"]<30)

#remove the selected mgs
dades.abpm=dades.abpm[,!names(dades.abpm)%in%mgs.filter]
dades.non=dades.non[,!names(dades.non)%in%mgs.filter]
##################################################################


#saving the final phenotype data.frame
write.table(dades.abpm, file=paste(output.folder,'/dades.pheno_malmo_802_uppsala_3261_new.csv',sep=""),
            col.names=T,row.names=F,sep=",")

write.table(dades.non, file=paste(output.folder,'/dades.pheno_malmo_2866_uppsala_395_validation.csv',sep=""),
            col.names=T,row.names=F,sep=",")
